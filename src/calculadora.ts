import type { DadosRescisao, ResultadoRescisao, ErroValidacao } from './types.js';
...
function calcularMesesProporcionais(inicio: Date, fim: Date): number {
...
function calcularAvisoPrevio(salarioBase: number, dataAdmissao: Date, dataDemissao: Date): number {
  const anosTrabalhados = Math.floor(
    (dataDemissao.getTime() - dataAdmissao.getTime()) / (365.25 * 24 * 60 * 60 * 1000)
  );
...
function validarDados(dados: DadosRescisao): ErroValidacao[] {
...
export function calcularRescisao(dados: DadosRescisao): { resultado: ResultadoRescisao | null; erros: ErroValidacao[] } {
  ...
  const resultado: ResultadoRescisao = {
...
export type { DadosRescisao, ResultadoRescisao, ErroValidacao } from './types.js';
